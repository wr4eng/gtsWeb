    ...
    */
    ...
    public function buildHtmlOptions()
    {
        ...
        $htmlOptions = array(
            ...
            'data-pk'   => (isset($this->model->ident)?$this->model->ident:$this->model->primaryKey),
        );
    ...
    {
        if ((isset($this->model->primaryKey)) && (is_array($this->model->primaryKey))) {
            $ident = "";
            foreach($this->model->primaryKey as $value)
                $ident .= $value;
            return get_class($this->model) . '_' . $this->attribute . $ident;
        } else
            return get_class($this->model) . '_' . $this->attribute . ($this->model->primaryKey ? '_' . str_replace("@","",$this->model->primaryKey) : '_new');
    }
